...
public class RegionModifyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_region_modify); // 연령대 수정 레이아웃 연결

        // 뷰 초기화
        backButton = findViewById(R.id.backButton);
        completionButton = findViewById(R.id.Completion_button); // XML ID 확인 후 수정
        RegionInput = findViewById(R.id.textInput); // XML ID 확인 후 수정
        ...
        completionButton.setOnClickListener(v -> {
            String teamRegion = RegionInput.getText().toString();
            if (!teamRegion.isEmpty()) {
                changeTeamRegion(teamRegion, accessToken, teamId); // 연령대 수정 요청
            } else {
                Toast.makeText(RegionModifyActivity.this, "팀 활동지역을 입력해주세요.", Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void changeTeamRegion(String teamRegion, String accessToken, Long teamId) {
        String authorizationHeader = "Bearer " + accessToken;

        // UpdateTeamRequest 객체 생성 (st와 number 필드 포함)
        UpdateTeamRequest request = new UpdateTeamRequest(teamRegion, 0); // number는 임의로 0 설정

        teamApiService.updateTeamRegion(authorizationHeader, teamId, request).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                if (response.isSuccessful()) {
                    Toast.makeText(RegionModifyActivity.this, "팀 활동지역이 성공적으로 변경되었습니다.", Toast.LENGTH_SHORT).show();
                    finish(); // 액티비티 종료
                } else {
                    Log.e("API Error", "Error response code: " + response.code());
                    Toast.makeText(RegionModifyActivity.this, "서버 오류가 발생했습니다.", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                ...
                Toast.makeText(RegionModifyActivity.this, "네트워크 오류가 발생했습니다.", Toast.LENGTH_SHORT).show();
            }
        });
    }
